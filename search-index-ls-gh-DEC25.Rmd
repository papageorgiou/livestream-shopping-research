...
live_shop_grouped_by_month <-  live_shop_grouped_by_month %>% bind_rows(df_to_join) %>% 
  mutate(
...
sub_shop <- "Average monthly search activity is up ~50% compared to 2024. This follows a slowdown after Meta discontinued live shopping features in 2022-2023
...
ggplot(index_df, aes(x = month_end, y = index_value)) +
  ...
  theme(
    plot.caption = element_text(hjust = 0.3, size=15, face="italic"),   # 0 = left, 0.5 = center, 1 = right
    #plot.caption.position = "plot",           # place caption against the plot edge
    plot.title = element_markdown(lineheight = 1.05)
  )
...
```{r}
save_for_report(
  filename = "live_shopping_annot_600dpi_dec2025.png",  
  width_mm = 450, 
  height_mm = 200, 
  dpi = 600
)
