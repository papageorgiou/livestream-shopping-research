...
reticulate::use_condaenv("genenv")
```
...
## Addition of recent months (July - Nov 2025)

Search volumes from live Google keyword planner tool (csv version available in repo)

```{r}
jul_nov_25 <- c(6160,	5560,	5560,	7250,	7230)
...
live_shop_grouped_by_month_48 <- live_shop_all_by_month %>%
  group_by(month) %>% 
  summarise(total_monthly_searches = sum(searches_past_months)) 


month <- seq(ymd("2025-07-01"), by = "month", length.out = 5)
total_monthly_searches <- jul_nov_25


df_to_join <- tibble(month, total_monthly_searches)

live_shop_grouped_by_month <-  live_shop_grouped_by_month_48 %>% bind_rows(df_to_join) %>% 
  mutate(
...
```{r}
title_shop <- "US Live Shopping Regains Momentum in 2025"
...
sub_shop <- "Avg. monthly searches are up ~ 50% compared to 2024, following a slowdown after Meta discontinued live shopping in 2022. \nMetric: Search interest index for Live Shopping (LS), July 2021 = 100"


sub_shop_2 <- "Following a slowdown after Meta discontinued live shopping features in 2022-2023 
\nMetric: Search interest index for Live Shopping (LS), July 2021 = 100"
# sub_shop <- "INTERNAL USE: Unfiltered graph includes search volumes subject to potential bot activity in certain months"

caption_shop <- "\n\nIndex based on normalized U.S. search volume across 188 live shopping-related queries (July 2021 = 100). \nSource: Google Ads API (keyword planning service), July 2021-November 2025. Data retrieved in July 2025 and December 2025." 
...
ggplot(index_df, aes(x = month_end, y = index_value)) +
  ...
  labs(
    ...
    x = NULL, 
    y = NULL, 
    caption = str_wrap(caption_shop,150)
  ) +  
  scale_y_continuous(limits = c(0, NA)) + 
  ...
  theme(
    plot.caption = element_text(hjust = 0, size=16, face="italic", margin = margin(t=20)),   # 0 = left, 0.5 = center, 1 = right
    plot.caption.position = "panel",           # place caption against the plot edge
    plot.title = element_markdown(lineheight = 1.05), plot.subtitle = element_text(size=21),
...
```{r}
save_for_report(
  filename = "live_shopping_annot_600dpi_dec2025-b.png",  
  width_mm = 425, 
  height_mm = 200, 
  dpi = 600
)
